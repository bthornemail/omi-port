# OMI-Port Status

## v0 — Dormant Port Notation Transform

Status: **Complete**

### What Exists

- `include/omi_port.h` — self-contained C ABI header
- `src/omi_port.c` — C99 implementation (zero I/O, zero authority)
- `tests/test_scope.c` — 10+ checks for URI-CIDR scope parser
- `tests/test_transform.c` — 15+ checks for PortTensor_G transform
- `tests/test_notation.c` — 20+ checks for chirality + init + LL/MM/NN absence
- `tests/test_authority_negative.c` — 12+ checks proving v0 queries return 0 even with flags = 1
- `tests/test_harness.h` — shared CHECK macros
- `tests/golden/` — 4 golden fixture files
- `canon/OMI/Port.hs` — normative Haskell EDSL reference
- `lisp/port-declaration.omi` — dormant declaration examples
- `text/PORT_TEXT.md` — text notation reference
- `docs/` — 8 docs covering handoff, contract, ABI, scope, transform, notation, authority, test matrix
- `Makefile` — `make`, `make test`, `make clean`, `make hs-check`

### Build Status

All four test binaries build warning-free and pass.

### Boundary

- No omi-lisp import
- No omi-protocol import
- No I/O in library
- All authority flags default to 0
